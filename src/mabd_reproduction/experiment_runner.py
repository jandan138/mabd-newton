...
from .spinning_box_contact_collision_gate_candidate import (
...
def run_rolling_spinning_rbd_no_slip_reference(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
    ...
    if config.report_status != EvidenceStatus.INCOMPLETE:
        raise ValueError(
            "Phase 79 rolling/spinning no-slip reference requires incomplete report status"
        )
    report_path = _resolve_output_path(
    ...
    report = write_rolling_spinning_rbd_no_slip_reference_report(
        report_path,
        config=config,
        source_commit=source_commit,
        vendored_newton_commit=vendored_newton_commit,
...
def run_rolling_spinning_rbd_explicit_no_slip_candidate(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
    ...
    if config.report_status != EvidenceStatus.INCOMPLETE:
        raise ValueError(
            "Phase 80 rolling/spinning explicit no-slip candidate requires incomplete report status"
        )
    report_path = _resolve_output_path(
    ...
    report = write_rolling_spinning_rbd_explicit_no_slip_candidate_report(
        report_path,
        config=config,
        source_commit=source_commit,
        vendored_newton_commit=vendored_newton_commit,
...
def run_rolling_spinning_mabd_rolling_contact_candidate(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
    ...
    if config.report_status != EvidenceStatus.INCOMPLETE:
        raise ValueError(
            "Phase 81 rolling/spinning M-ABD rolling-contact candidate "
    ...
    report = write_rolling_spinning_mabd_rolling_contact_candidate_report(
        report_path,
        config=config,
        source_commit=source_commit,
        vendored_newton_commit=vendored_newton_commit,
...
def run_rolling_spinning_paper_faithful_gate_ledger(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
    ...
    report_path = _resolve_output_path(
        config.paper_faithful_gate_ledger.output_report,
        output_path=output_path,
        output_root=output_root,
    )
    report = write_rolling_spinning_paper_faithful_gate_ledger_report(
        report_path,
        config=config,
        source_commit=source_commit,
        vendored_newton_commit=vendored_newton_commit,
...
def run_rolling_spinning_rbd_explicit_source_gate(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
...
def run_rolling_spinning_mabd_source_gate(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
    ...
    report = write_rolling_spinning_mabd_source_gate_report(
        report_path,
        config=config,
        source_commit=source_commit,
        vendored_newton_commit=vendored_newton_commit,
...
def run_rolling_spinning_timing_source_gate(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
    ...
    report = write_rolling_spinning_timing_source_gate_report(
        report_path,
        config=config,
        source_commit=source_commit,
        vendored_newton_commit=vendored_newton_commit,
...
def run_spinning_box_development_comparison(
    *,
    config_path: str | Path,
    matrix_path: str | Path,
    source_commit: str,
    ...
    if config.report_status != EvidenceStatus.INCOMPLETE:
        raise ValueError(
            "spinning_box_development_comparison requires incomplete report status"
        )
    report_path = _resolve_output_path(
...
def run_spinning_box_contact_collision_gate_candidate(
    *,
    ...
    if config.report_status != EvidenceStatus.INCOMPLETE:
        raise ValueError(
            "spinning_box_contact_collision_gate_candidate requires incomplete report status"
        )
    report_path = _resolve_output_path(
        config.contact_collision_gate_candidate.output_report,
        output_path=output_path,
        output_root=output_root,
    )
    report = write_spinning_box_contact_collision_gate_candidate_report(
        report_path,
        config=config,
        source_commit=source_commit,
        vendored_newton_commit=vendored_newton_commit,
