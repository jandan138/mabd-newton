...
from .paper_source_audit import (
...
ROLLING_SPINNING_EXPLICIT_NO_SLIP_CANDIDATE_REQUIRED_REPRODUCTION_GAPS = [
    "paper_faithful_explicit_rbd_baseline",
    "paper_faithful_implicit_rbd_baseline",
    "paper_faithful_mabd_rolling_cylinder",
    "paper_comparable_timing",
]
ROLLING_SPINNING_EXPLICIT_NO_SLIP_CANDIDATE_BLOCKING_REASONS = [
...
ROLLING_SPINNING_MABD_ROLLING_CONTACT_CANDIDATE_REQUIRED_REPRODUCTION_GAPS = [
    "paper_faithful_explicit_rbd_baseline",
    "paper_faithful_implicit_rbd_baseline",
    "paper_faithful_mabd_rolling_cylinder",
    "paper_comparable_timing",
]
ROLLING_SPINNING_MABD_ROLLING_CONTACT_CANDIDATE_BLOCKING_REASONS = [
...
ROLLING_SPINNING_RBD_EXPLICIT_SOURCE_GATE_BLOCKING_REASONS = [
    "paper_faithful_explicit_rbd_baseline_missing",
    "paper_faithful_implicit_rbd_baseline_missing",
    "paper_faithful_mabd_collision_missing",
    "paper_comparable_timing_missing",
]
ROLLING_SPINNING_RBD_IMPLICIT_SOURCE_GATE_BLOCKING_REASONS = [
    "paper_faithful_implicit_rbd_baseline_missing",
    "paper_faithful_explicit_rbd_baseline_missing",
    "paper_faithful_mabd_collision_missing",
    "paper_comparable_timing_missing",
]
ROLLING_SPINNING_MABD_SOURCE_GATE_BLOCKING_REASONS = [
    "paper_faithful_mabd_collision_missing",
    "paper_faithful_explicit_rbd_baseline_missing",
    "paper_faithful_implicit_rbd_baseline_missing",
    "paper_comparable_timing_missing",
...
class RollingCylinderMABDNewtonResult:
    ...
    generated_world_constraint_count_summary: dict[str, int]
    generated_plane_constraint_count_summary: dict[str, int]
...
def _int_count_summary(counts: list[int]) -> dict[str, int]:
    if not counts:
        return {"initial": 0, "final": 0, "min": 0, "max": 0}
    return {
        "initial": counts[0],
        "final": counts[-1],
        "min": min(counts),
...
def run_rolling_cylinder_rbd_explicit_no_slip_candidate(
    config: RollingSpinningRBDBaselineConfig,
) -> RollingCylinderNoSlipReferenceResult:
    mass_kg, inertia_diag_kg_m2 = _rolling_cylinder_mass_and_inertia(config)
    position = np.asarray(config.initial_position_m, dtype=float).copy()
    linear_velocity = np.asarray(config.initial_linear_velocity_m_s, dtype=float).copy()
    angular_velocity = np.asarray(config.initial_angular_velocity_rad_s, dtype=float).copy()
    ...
    for step_index in range(config.step_count + 1):
        ...
        if step_index == config.step_count:
            break
        position = np.asarray(
            position + linear_velocity * config.time_step_s,
            dtype=float,
        )
        position[1] = config.radius_m
        linear_velocity[1:] = 0.0
        angular_velocity[:2] = 0.0
        angular_velocity[2] = -linear_velocity[0] / config.radius_m
    total_wall_time_ms = (perf_counter() - start) * 1000.0

    final_position = np.asarray(position, dtype=float)
    final_linear_velocity = np.asarray(linear_velocity, dtype=float)
    final_angular_velocity = np.asarray(angular_velocity, dtype=float)
    final_energy = _rolling_cylinder_energy(
        mass_kg=mass_kg,
        inertia_diag_kg_m2=inertia_diag_kg_m2,
        gravity_m_s2=config.gravity_m_s2,
...
def run_rolling_cylinder_mabd_newton(
    ...
    contact_counts: list[int] = []
    generated_world_constraint_counts: list[int] = []
    generated_plane_constraint_counts: list[int] = []
    ...
    if config.contact_constraint_mode == "world":
    ...
    with redirect_stdout(sys.stderr):
        ...
        for step_index in range(config.step_count + 1):
            ...
            if contacts_summary is None:
                generated_world_constraint_counts.append(0)
                generated_plane_constraint_counts.append(0)
            else:
                generated_world_constraint_counts.append(
                    int(contacts_summary.generated_world_constraint_count)
                )
                generated_plane_constraint_counts.append(
    ...
    return RollingCylinderMABDNewtonResult(
        ...
        generated_world_constraint_count_summary=_int_count_summary(
            generated_world_constraint_counts
        ),
        generated_plane_constraint_count_summary=_int_count_summary(
            generated_plane_constraint_counts
        ),
...
def write_rolling_spinning_rbd_explicit_no_slip_candidate_report(
    path: str | Path,
    *,
    config: RollingSpinningRunConfig,
    source_commit: str,
    ...
    lane_config = config.rbd_explicit_no_slip_candidate
    result = run_rolling_cylinder_rbd_explicit_no_slip_candidate(lane_config)
    thresholds = dict(lane_config.thresholds)
    thresholds["max_center_height_drift_m"] = 1.0e-12
    threshold_violations: list[str] = []
    if result.no_slip_residual_m_s > thresholds["max_no_slip_residual_m_s"]:
    ...
    expected = {
        "paper_claim_status": (
            "local explicit no-slip rolling-cylinder candidate generated; this is "
            "not the paper explicit RBD solver, M-ABD contact/friction, or "
        ...
        "known_source_gaps": [
            "paper does not specify cylinder dimensions, mass, or initial state",
            "paper exact explicit and implicit RBD solver details are not available",
            "paper no-slip condition is inferred from the rolling-cylinder setup",
            "local no-slip projection is not a contact dynamics solve",
            "paper affine rolling contact/friction solve is not yet implemented",
            "paper-comparable i7 single-thread timing is not measured by this lane",
        ],
    }
    observed = {
        "candidate_status": "local_no_slip_projection_generated",
        "local_runtime_measured": True,
        "paper_comparable": False,
        "full_experiment_claim_passed": False,
        "required_reproduction_gaps_remaining": list(
    ...
    report = ClaimReport(
        ...
        status=result.status,
        failure_reason=(
            "Local explicit no-slip projection candidate only; paper-faithful "
            "explicit RBD, implicit RBD, M-ABD rolling-cylinder contact/friction, "
            "and paper-comparable timing evidence remain missing"
        ),
        timing_distribution={
            "total_wall_time_ms": result.total_wall_time_ms,
            "step_count": result.step_count,
            "paper_comparable": False,
            "scope": "local_no_slip_projection_not_paper_timing",
            "paper_explicit_rbd_total_simulation_time_ms": (
...
def write_rolling_spinning_mabd_rolling_contact_candidate_report(
    path: str | Path,
    *,
    config: RollingSpinningRunConfig,
    source_commit: str,
    ...
    candidate_config = config.mabd_rolling_contact_candidate
    result = run_rolling_cylinder_mabd_newton(candidate_config)
    thresholds = candidate_config.thresholds
    threshold_violations: list[str] = []
    if result.no_slip_residual_m_s > thresholds["max_no_slip_residual_m_s"]:
        threshold_violations.append("max_no_slip_residual_m_s")
    if result.relative_energy_drift > thresholds["max_relative_energy_drift"]:
    ...
    expected = {
        "paper_claim_status": (
            "M-ABD rolling-contact world-constraint candidate only; full paper "
            "claim requires paper-faithful affine rolling contact/friction, RBD "
            "baselines, and paper-comparable timing"
        ),
        "source_lines": list(config.source_lines),
        "config_path": ROLLING_SPINNING_CONFIG_PATH,
        "canonical_python": CANONICAL_PYTHON,
        ...
        "full_experiment_claim_passed": False,
        "known_source_gaps": [
            "diagnostic world constraints are not the paper rolling friction law",
            "paper affine rolling-cylinder contact details are still missing",
            "paper-faithful explicit and implicit RBD baselines are not yet recorded",
            "paper-comparable i7 single-thread timing protocol is not yet measured",
        ],
    }
    observed = {
        "rolling_contact_candidate_status": (
            "world_constraint_candidate_incomplete"
        ),
        "lane_status": "incomplete_rolling_contact_candidate_generated",
        "local_runtime_measured": True,
        "paper_comparable": False,
        "full_experiment_claim_passed": False,
        "required_reproduction_gaps": list(
            ROLLING_SPINNING_MABD_ROLLING_CONTACT_CANDIDATE_REQUIRED_REPRODUCTION_GAPS
        ),
        "required_reproduction_gaps_remaining": list(
            ROLLING_SPINNING_MABD_ROLLING_CONTACT_CANDIDATE_REQUIRED_REPRODUCTION_GAPS
        ...
        + [
            ...
            "MABDCPUOracleConfig.contact_constraint_mode",
            "SolverMABD.last_contacts_input_summary",
        ],
        "newton_device": "cpu",
        "solver_name": "newton.solvers.SolverMABD",
        "solver_scope": (
            "mabd_affine_cylinder_static_plane_world_constraint_candidate_"
            "not_paper_faithful"
        ),
        "contact_constraint_mode": candidate_config.contact_constraint_mode,
        "contacts_input_summary_source": result.contacts_input_summary_source,
        "generated_world_constraint_count_summary": dict(
            result.generated_world_constraint_count_summary
        ),
        "generated_plane_constraint_count_summary": dict(
            result.generated_plane_constraint_count_summary
        ),
        "young_modulus_pa": candidate_config.young_modulus_pa,
        "poisson_ratio": candidate_config.poisson_ratio,
        "zero_stiffness_diagnostic": candidate_config.zero_stiffness_diagnostic,
        "step_count": result.step_count,
        "time_step_s": result.time_step_s,
        "radius_m": result.radius_m,
        "half_height_m": result.half_height_m,
    ...
    report = ClaimReport(
        ...
        status=result.status,
        failure_reason=(
            "SolverMABD world-constraint rolling-contact candidate only; paper-"
            "faithful affine rolling contact/friction, RBD baselines, and "
            "paper-comparable timing evidence remain missing"
        ),
        timing_distribution={
            "total_wall_time_ms": result.total_wall_time_ms,
...
def write_rolling_spinning_rbd_explicit_source_gate_report(
    path: str | Path,
    *,
    config: RollingSpinningRunConfig,
    source_commit: str,
    ...
    gate_config = config.rbd_explicit_source_gate
    source_audit = rolling_spinning_explicit_rbd_source_audit()
    blocking_reasons = list(
        dict.fromkeys(
            [
    ...
    expected = {
        "paper_claim_status": (
            "fail-closed source gate only; the paper source must disclose the "
            "rolling-cylinder explicit RBD setup before this can be treated as "
        ...
        "required_gate": "paper_faithful_explicit_rbd_baseline",
        "required_gate_status": "passed",
        "paper_comparable": False,
        "full_experiment_claim_passed": False,
    }
    ...
    report = ClaimReport(
        ...
        status=EvidenceStatus.INCOMPLETE,
        failure_reason=(
            "Paper source audit only; public source does not provide enough "
            "rolling-cylinder explicit RBD setup details to pass the "
            "paper-faithful explicit RBD baseline gate"
        ),
        timing_distribution={
            "status": "not_measured",
            "paper_comparable": False,
...
def write_rolling_spinning_rbd_implicit_source_gate_report(
    path: str | Path,
    *,
    config: RollingSpinningRunConfig,
    source_commit: str,
    ...
    gate_config = config.rbd_implicit_source_gate
    source_audit = rolling_spinning_implicit_rbd_source_audit()
    blocking_reasons = list(
        dict.fromkeys(
            [
                *source_audit.blockers,
    ...
    expected = {
        "paper_claim_status": (
            "fail-closed source gate only; the paper source must disclose the "
            "rolling-cylinder implicit RBD setup before this can be treated as "
            "a paper-faithful baseline"
        ),
        "source_lines": list(config.source_lines),
        "config_path": ROLLING_SPINNING_CONFIG_PATH,
        ...
        "paper_hardware_context": config.performance.paper_hardware_context,
        "required_source_parameters": list(gate_config.required_source_parameters),
        "required_gate": "paper_faithful_implicit_rbd_baseline",
        "required_gate_status": "passed",
        "paper_comparable": False,
        "full_experiment_claim_passed": False,
    }
    ...
    report = ClaimReport(
        ...
        solver_mode="rolling_spinning_implicit_rbd_source_gate",
        backend="paper_source_audit",
        ...
        failure_reason=(
            "Paper source audit only; public source does not provide enough "
            "rolling-cylinder implicit RBD setup details to pass the "
            "paper-faithful implicit RBD baseline gate"
        ),
        timing_distribution={
            "status": "not_measured",
            "paper_comparable": False,
...
def write_rolling_spinning_mabd_source_gate_report(
    path: str | Path,
    *,
    config: RollingSpinningRunConfig,
    source_commit: str,
    ...
    gate_config = config.mabd_source_gate
    source_audit = rolling_spinning_mabd_source_audit()
    blocking_reasons = list(
        dict.fromkeys(
            [
                *source_audit.blockers,
    ...
    expected = {
        "paper_claim_status": (
            "fail-closed source gate only; the paper source must disclose the "
            "rolling-cylinder M-ABD affine contact and collision setup before "
            "this can be treated as a paper-faithful M-ABD rolling baseline"
        ),
        "source_lines": list(config.source_lines),
        "config_path": ROLLING_SPINNING_CONFIG_PATH,
        "canonical_python": CANONICAL_PYTHON,
        ...
        "paper_hardware_context": config.performance.paper_hardware_context,
        "required_source_parameters": list(gate_config.required_source_parameters),
        "required_gate": "paper_faithful_mabd_rolling_cylinder",
        "required_gate_status": "passed",
        "paper_comparable": False,
        "full_experiment_claim_passed": False,
    }
    ...
    report = ClaimReport(
        ...
        failure_reason=(
            "Paper source audit only; public source does not provide enough "
            "rolling-cylinder M-ABD affine contact and collision setup details "
            "to pass the paper-faithful M-ABD rolling-cylinder gate"
        ),
        timing_distribution={
            "status": "not_measured",
            "paper_comparable": False,
...
__all__ = [
    ...
    "run_rolling_cylinder_rbd_explicit_no_slip_candidate",
    "run_rolling_cylinder_rbd_implicit_baseline",
    "run_rolling_cylinder_rbd_no_slip_reference",
    ...
    "write_rolling_spinning_mabd_source_gate_report",
