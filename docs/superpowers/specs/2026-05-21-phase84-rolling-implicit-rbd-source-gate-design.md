...
Phase 84 adds a fail-closed source-audit gate for the rolling/spinning
`paper_faithful_implicit_rbd_baseline` requirement.
...
It audits the public paper source for the rolling-cylinder implicit RBD details
needed before any paper-faithful implicit RBD baseline can be claimed.
...
implicit RBD setup to support a paper-faithful baseline. Required source
parameters are:
...
- `backend = paper_source_audit`
- `status = incomplete`
- `observed.source_audit_status = implicit_rbd_source_requirements_incomplete`
...
- Config validation covers `rbd_implicit_source_gate`.
...
- `docs/reference/reproduction-gap-audit.yaml` records the Phase84 report and
