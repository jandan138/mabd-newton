...
- docs/provenance introduction commit:
  `1df81a6`
- local patch files:
...
- static-geometry guard:
  generated rows require the opposite shape to have `shape_body == -1`;
...
- supported row shape:
  ...
  body id and then through `mabd:body_index` to one M-ABD body row, while the
  opposite side has `shape_body == -1`
- unsupported rows:
  invalid shapes, rows with neither side mapped, rows with both sides mapped,
  rows with a dynamic non-M-ABD body on the opposite side, and rows beyond
  `rigid_contact_max` are skipped and counted
- normal convention:
...
- `PYTHONPATH=vendor/newton /cpfs/user/zhuzihou/conda-managed/envs/mabd-newton-py310/bin/python -m unittest newton.tests.test_mabd_phase4_solver_step`
  - result after review fix: `Ran 35 tests ... OK`
- `PYTHONPATH=src:vendor/newton /cpfs/user/zhuzihou/conda-managed/envs/mabd-newton-py310/bin/python -m unittest tests.test_mabd_phase4_solver_step`
  - result after review fix: `Ran 60 tests ... OK`

Review-fix red/green checks:

- `PYTHONPATH=vendor/newton /cpfs/user/zhuzihou/conda-managed/envs/mabd-newton-py310/bin/python -m unittest newton.tests.test_mabd_phase4_solver_step.MABDPhase4InternalTests.test_solver_step_skips_dynamic_non_mabd_contact_rows`
  - red result before review fix:
    `AssertionError: 2 != 0`
- `PYTHONPATH=vendor/newton /cpfs/user/zhuzihou/conda-managed/envs/mabd-newton-py310/bin/python -m unittest newton.tests.test_mabd_phase4_solver_step.MABDPhase4InternalTests.test_solver_step_skips_dynamic_non_mabd_contact_rows newton.tests.test_mabd_phase4_solver_step.MABDPhase4InternalTests.test_solver_step_consumes_newton_contacts_as_plane_constraints newton.tests.test_mabd_phase4_solver_step.MABDPhase4InternalTests.test_solver_step_flips_contact_normal_when_mabd_body_is_shape1`
  - result: `Ran 3 tests ... OK`
- `PYTHONPATH=src:vendor/newton /cpfs/user/zhuzihou/conda-managed/envs/mabd-newton-py310/bin/python -m unittest tests.test_mabd_phase4_solver_step.MABDPhase4SolverStepTests.test_solver_step_skips_dynamic_non_mabd_contact_rows tests.test_mabd_phase4_solver_step.MABDPhase4SolverStepTests.test_solver_step_consumes_newton_contacts_as_plane_constraints tests.test_mabd_phase4_solver_step.MABDPhase4SolverStepTests.test_solver_step_flips_contact_normal_when_mabd_body_is_shape1`
  - result: `Ran 3 tests ... OK`

## Metrics And Thresholds

- static contact generated plane constraints:
  `generated_plane_constraint_count == 1`
- dynamic non-M-ABD generated plane constraints:
  `generated_plane_constraint_count == 0`
- dynamic non-M-ABD skipped contacts:
  `skipped_contact_count == 2`
- overflow accounting threshold:
  `rigid_contact_overflow_count == max(0, rigid_contact_count - rigid_contact_capacity)`
- tolerance policy:
  exact integer equality for counts; state parity with explicit
  `MABDCPUOraclePlaneConstraint` uses `atol=1.0e-7`
- raw artifacts: `not applicable`; no generated videos, simulation run
  directories, or large logs
...
collision detection, not body-body affine contact, not dynamic non-M-ABD body
contact, not generic inequality-constrained M-ABD KKT, not paper-faithful affine collision/contact,
