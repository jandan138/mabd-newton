...
class Phase0BootstrapTests(unittest.TestCase):
    ...
    def assert_latest_update_is_phase86_or_later(
        ...
        expected_updates = {
            ...
            "phase89_spinning_box_contact_collision_gate_candidate": {
                ...
                "update_date": "2026-05-21",
                "source_commit": (
                    validate_docs.PHASE89_SPINNING_BOX_CONTACT_COLLISION_GATE_CANDIDATE_COMMIT
                ),
                "report": (
                    validate_docs.SPINNING_BOX_CONTACT_COLLISION_GATE_CANDIDATE_REPORT_PATH
                ),
                "report_sha256": (
                    validate_docs.PHASE89_SPINNING_BOX_CONTACT_COLLISION_GATE_CANDIDATE_SHA256
                ),
                "status": "incomplete",
            },
        }
    ...
    def test_phase79_rolling_cylinder_no_slip_reference_artifact(self) -> None:
        import scripts.validate_docs as validate_docs

        boundary_text = (ROOT / "docs/reference/claim-boundaries.md").read_text()
        current = validate_docs.claim_boundary_bullet(
            boundary_text,
            "This repository contains Phase 79",
        )
        verified = validate_docs.claim_boundary_bullet(boundary_text, "Phase 79 verifies")
        non_claim = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 79 does not verify",
        )
        forbidden = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 79 analytic no-slip reference evidence",
        )

        self.assertIn("analytic no-slip rolling-cylinder reference", current)
        self.assertIn("single_body_rolling_spinning_rbd_no_slip_reference.json", verified)
        self.assertIn("paper_comparable = false", verified)
        ...
        report = load_claim_report(
            ROOT / validate_docs.ROLLING_SPINNING_RBD_NO_SLIP_REFERENCE_REPORT_PATH
        )
        self.assertEqual(
            report.source_commit,
        ...
        self.assertEqual(report.vendored_newton_commit, "96713fa965463b69c229a4d30582c733ff3526bb")
        self.assertEqual(report.claim_id, "experiment.single_body.rolling_spinning")
        self.assertEqual(report.baseline_lane, "rbd_no_slip_reference")
        self.assertEqual(
            report.solver_mode,
            "analytic_no_slip_rolling_cylinder_reference",
        )
        self.assertEqual(report.backend, "cpu_numpy_closed_form")
        self.assertEqual(report.status.value, "incomplete")
        self.assertFalse(report.expected["paper_comparable"])
        self.assertFalse(report.expected["full_experiment_claim_passed"])
        self.assertFalse(report.observed["paper_comparable"])
        self.assertFalse(report.observed["full_experiment_claim_passed"])
        self.assertEqual(
            report.observed["reference_status"],
            "analytic_no_slip_reference_generated",
        )
        self.assertFalse(report.observed["local_runtime_measured"])
        self.assertEqual(report.observed["threshold_violations"], [])
        self.assertEqual(report.observed["final_position_m"], [100.0, 0.5, 0.0])
        self.assertLessEqual(
        ...
        self.assertFalse(report.timing_distribution["paper_comparable"])
        self.assertEqual(report.timing_distribution["status"], "not_measured")
        self.assertNotIn("total_wall_time_ms", report.timing_distribution)
        self.assertEqual(report.raw_outputs, {})
        self.assertEqual(report.plot_paths, {})
        ...
        self.assertEqual(
            gap_entry["rbd_no_slip_reference_report_sha256"],
            validate_docs.PHASE79_ROLLING_SPINNING_RBD_NO_SLIP_REFERENCE_SHA256,
        )
        self.assertEqual(
            gap_entry["remaining_reproduction_gaps_after_phase79"],
            [
                "paper_faithful_explicit_rbd_baseline",
                "paper_faithful_implicit_rbd_baseline",
                "paper_faithful_mabd_rolling_cylinder",
        ...
        self.assertIn("paper-faithful explicit RBD", gap_entry["next_action"])
        self.assertIn("paper-comparable timing", gap_entry["next_action"])
        validate_docs.validate_phase79_record()

    def test_phase79_record_has_required_evidence_fields(self) -> None:
        import scripts.validate_docs as validate_docs
        ...
        for snippet in (
            ...
            validate_docs.ROLLING_SPINNING_RBD_NO_SLIP_REFERENCE_REPORT_PATH,
            validate_docs.PHASE79_ROLLING_SPINNING_RBD_NO_SLIP_REFERENCE_SHA256,
            "analytic_no_slip_rolling_cylinder_reference",
    ...
    def test_phase84_rolling_implicit_rbd_source_gate_artifact(self) -> None:
        import scripts.validate_docs as validate_docs
        ...
        current = validate_docs.claim_boundary_bullet(
            boundary_text,
            "This repository contains Phase 84",
        )
        verified = validate_docs.claim_boundary_bullet(boundary_text, "Phase 84 verifies")
        non_claim = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 84 does not verify",
        )
        forbidden = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 84 rolling/spinning implicit RBD source gate evidence",
        )

        self.assertIn("implicit RBD source gate", current)
        self.assertIn(
        ...
        self.assertIn("paper_faithful_gate_passed = false", verified)
        self.assertIn("paper_implicit_rbd_solver_details_missing_from_public_source", verified)
        self.assertIn("full_experiment_claim_passed = false", verified)
        for snippet in (
            "paper-faithful implicit RBD",
            "paper-faithful explicit RBD",
        ...
        verified_paths = set(validate_docs.REQUIRED_PATHS)
        self.assertIn(
            validate_docs.ROLLING_SPINNING_RBD_IMPLICIT_SOURCE_GATE_REPORT_PATH,
            verified_paths,
        )

        report = load_claim_report(
            ROOT / validate_docs.ROLLING_SPINNING_RBD_IMPLICIT_SOURCE_GATE_REPORT_PATH
        )
        self.assertEqual(
            report.source_commit,
            validate_docs.PHASE84_ROLLING_IMPLICIT_RBD_SOURCE_GATE_COMMIT,
        )
        self.assertEqual(report.vendored_newton_commit, "96713fa965463b69c229a4d30582c733ff3526bb")
        self.assertEqual(report.claim_id, "experiment.single_body.rolling_spinning")
        self.assertEqual(report.baseline_lane, "rbd_implicit_source_gate")
        self.assertEqual(report.solver_mode, "rolling_spinning_implicit_rbd_source_gate")
        self.assertEqual(report.backend, "paper_source_audit")
        self.assertEqual(report.status.value, "incomplete")
        self.assertEqual(report.expected["required_source_parameters"], required_parameters)
        self.assertEqual(report.expected["required_gate"], "paper_faithful_implicit_rbd_baseline")
        self.assertFalse(report.expected["paper_comparable"])
        self.assertFalse(report.expected["full_experiment_claim_passed"])
        self.assertEqual(
            report.observed["source_audit_status"],
        ...
        self.assertEqual(
            actual_sha,
            validate_docs.PHASE84_ROLLING_SPINNING_RBD_IMPLICIT_SOURCE_GATE_SHA256,
        )
        ...
        self.assertEqual(
            gap_entry["rbd_implicit_source_gate_report_sha256"],
            validate_docs.PHASE84_ROLLING_SPINNING_RBD_IMPLICIT_SOURCE_GATE_SHA256,
        )
        self.assertEqual(
            gap_entry["remaining_reproduction_gaps_after_phase84"],
            required_gaps,
        )
        validate_docs.validate_phase84_record()

    def test_phase85_rolling_mabd_source_gate_artifact(self) -> None:
        import scripts.validate_docs as validate_docs
        ...
        current = validate_docs.claim_boundary_bullet(
            boundary_text,
            "This repository contains Phase 85",
        )
        verified = validate_docs.claim_boundary_bullet(boundary_text, "Phase 85 verifies")
        non_claim = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 85 does not verify",
        )
        forbidden = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 85 rolling/spinning M-ABD source gate evidence",
        )

        self.assertIn("M-ABD source gate", current)
        self.assertIn(
        ...
        self.assertIn("full_experiment_claim_passed = false", verified)
        for snippet in (
            "paper-faithful M-ABD rolling-cylinder behavior",
            "paper-faithful affine M-ABD rolling contact/friction",
            "paper-faithful explicit RBD",
            "paper-faithful implicit RBD",
            "paper-comparable timing",
            ...
            self.assertIn(snippet, non_claim)
        for snippet in (
            "passed M-ABD rolling-cylinder gate",
            "paper-faithful M-ABD rolling-cylinder result",
            "paper-faithful M-ABD collision result",
            "paper-comparable timing result",
            "completed rolling/spinning reproduction",
        ):
            self.assertIn(snippet, forbidden)
        ...
        verified_paths = set(validate_docs.REQUIRED_PATHS)
        self.assertIn(
            validate_docs.ROLLING_SPINNING_MABD_SOURCE_GATE_REPORT_PATH,
            verified_paths,
        )

        report = load_claim_report(
            ROOT / validate_docs.ROLLING_SPINNING_MABD_SOURCE_GATE_REPORT_PATH
        )
        self.assertEqual(
            report.source_commit,
            validate_docs.PHASE85_ROLLING_MABD_SOURCE_GATE_COMMIT,
        )
        self.assertEqual(report.vendored_newton_commit, "96713fa965463b69c229a4d30582c733ff3526bb")
        self.assertEqual(report.claim_id, "experiment.single_body.rolling_spinning")
        self.assertEqual(report.baseline_lane, "mabd_source_gate")
        self.assertEqual(report.solver_mode, "rolling_spinning_mabd_source_gate")
        self.assertEqual(report.backend, "paper_source_audit")
        self.assertEqual(report.status.value, "incomplete")
        self.assertEqual(report.expected["required_source_parameters"], required_parameters)
        self.assertEqual(report.expected["required_gate"], "paper_faithful_mabd_rolling_cylinder")
        self.assertFalse(report.expected["paper_comparable"])
        self.assertFalse(report.expected["full_experiment_claim_passed"])
        self.assertEqual(
            report.observed["source_audit_status"],
        ...
        self.assertEqual(
            actual_sha,
            validate_docs.PHASE85_ROLLING_SPINNING_MABD_SOURCE_GATE_SHA256,
        )
        ...
        self.assertEqual(
            gap_entry["mabd_source_gate_report_sha256"],
            validate_docs.PHASE85_ROLLING_SPINNING_MABD_SOURCE_GATE_SHA256,
        )
        self.assertEqual(
            gap_entry["remaining_reproduction_gaps_after_phase85"],
            required_gaps,
        )
        validate_docs.validate_phase85_record()

    def test_phase86_rolling_timing_source_gate_artifact(self) -> None:
        import scripts.validate_docs as validate_docs
        ...
        current = validate_docs.claim_boundary_bullet(
            boundary_text,
            "This repository contains Phase 86",
        )
        verified = validate_docs.claim_boundary_bullet(boundary_text, "Phase 86 verifies")
        non_claim = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 86 does not verify",
        )
        forbidden = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 86 rolling/spinning timing source gate evidence",
        )

        self.assertIn("timing source gate", current)
        self.assertIn(
        ...
        self.assertIn("paper_timing_gate_passed = false", verified)
        self.assertIn(
        ...
        for snippet in (
            "paper-comparable timing",
            "same-hardware paper timing",
            "single-thread timing enforcement",
            "paper-faithful runtime inputs",
            "any passed `experiment.*` claim",
        ):
            self.assertIn(snippet, non_claim)
        for snippet in (
        ...
        report = load_claim_report(
            ROOT / validate_docs.ROLLING_SPINNING_TIMING_SOURCE_GATE_REPORT_PATH
        )
        self.assertEqual(
            report.source_commit,
        ...
        self.assertEqual(report.vendored_newton_commit, "96713fa965463b69c229a4d30582c733ff3526bb")
        self.assertEqual(report.claim_id, "experiment.single_body.rolling_spinning")
        self.assertEqual(report.baseline_lane, "timing_source_gate")
        self.assertEqual(report.solver_mode, "rolling_spinning_timing_source_gate")
        self.assertEqual(report.backend, "paper_source_audit")
        self.assertEqual(report.status.value, "incomplete")
        self.assertEqual(report.expected["required_source_parameters"], required_parameters)
        self.assertEqual(report.expected["required_gate"], "paper_comparable_timing")
        self.assertFalse(report.expected["paper_comparable"])
        self.assertFalse(report.expected["full_experiment_claim_passed"])
        self.assertEqual(
            report.observed["source_audit_status"],
        ...
        self.assertFalse(report.observed["paper_timing_gate_passed"])
        self.assertFalse(report.observed["paper_comparable"])
        self.assertFalse(report.observed["full_experiment_claim_passed"])
        self.assertEqual(report.observed["missing_parameters"], required_parameters)
        self.assertEqual(
        ...
        self.assertEqual(
            gap_entry["timing_source_gate_report_sha256"],
            validate_docs.PHASE86_ROLLING_SPINNING_TIMING_SOURCE_GATE_SHA256,
        )
        self.assertEqual(
            gap_entry["remaining_reproduction_gaps_after_phase86"],
            required_gaps,
        )
    ...
    def test_phase89_spinning_box_contact_collision_gate_candidate_artifact(
        self,
    ) -> None:
        import scripts.validate_docs as validate_docs
        ...
        current = validate_docs.claim_boundary_bullet(
            boundary_text,
            "This repository contains Phase 89",
        )
        verified = validate_docs.claim_boundary_bullet(boundary_text, "Phase 89 verifies")
        non_claim = validate_docs.claim_boundary_bullet(
            boundary_text,
            "Phase 89 does not verify",
        )

        self.assertIn("unilateral static-plane contact/collision gate candidate", current)
        self.assertIn(
            "single_body_spinning_box_contact_collision_gate_candidate.json",
            verified,
        )
        self.assertIn("unilateral_plane", verified)
        self.assertIn("dense_cpu_active_set_drop_tensile_plane_rows", verified)
        self.assertIn("paper_faithful = false", verified)
        self.assertIn("comparison_pass_gate_enabled = false", verified)
        for snippet in (
            "paper-faithful affine contact/collision",
            "body-body affine contact",
            "friction",
        ...
        report = load_claim_report(
            ROOT / validate_docs.SPINNING_BOX_CONTACT_COLLISION_GATE_CANDIDATE_REPORT_PATH
        )
        self.assertEqual(
            report.source_commit,
            validate_docs.PHASE89_SPINNING_BOX_CONTACT_COLLISION_GATE_CANDIDATE_COMMIT,
        ...
        self.assertEqual(report.observed["contact_constraint_mode"], "unilateral_plane")
        self.assertFalse(report.observed["paper_faithful"])
        self.assertFalse(report.observed["paper_comparable"])
        self.assertFalse(report.observed["full_experiment_claim_passed"])
        self.assertFalse(report.observed["comparison_pass_gate_enabled"])
        self.assertEqual(report.observed["duration_s"], 10.0)
        self.assertEqual(report.observed["time_step_s"], 0.01)
        self.assertEqual(report.observed["step_count"], 1000)
        self.assertEqual(report.observed["sample_count"], 101)
        self.assertEqual(len(report.observed["trajectory_samples"]), 101)
        self.assertEqual(report.observed["max_unilateral_plane_requested_count"], 4)
        self.assertEqual(report.observed["max_unilateral_plane_accepted_count"], 3)
        self.assertEqual(report.observed["max_unilateral_plane_rejected_count"], 0)
        self.assertEqual(report.observed["max_unilateral_plane_skipped_count"], 1)
        self.assertIn(
        ...
        self.assertEqual(
            gap_entry["contact_collision_gate_candidate_report_sha256"],
            validate_docs.PHASE89_SPINNING_BOX_CONTACT_COLLISION_GATE_CANDIDATE_SHA256,
        )
        validate_docs.validate_phase89_record()
    ...
    def test_docs_validator_accepts_phase0_contract(self) -> None:
        ...
        self.assertIn(
            (
                ...
                "/75/76/77/78/79/80/81/82/83/84/85/86/87/88/89 docs/provenance validation passed"
            ),
            result.stdout,
        )
