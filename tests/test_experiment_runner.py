...
class ExperimentRunnerTests(unittest.TestCase):
    ...
    def _write_short_spinning_box_contact_collision_gate_candidate_config(
        self,
        tmpdir: str,
    ) -> Path:
        config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
        config["contact_collision_gate_candidate"]["duration_s"] = 0.02
        config["contact_collision_gate_candidate"]["sample_count"] = 3
        path = (
            Path(tmpdir)
            / "single_body_spinning_box_short_contact_collision_gate_candidate.yaml"
        )
        path.write_text(yaml.safe_dump(config), encoding="utf-8")
        return path
    ...
    def _write_short_rolling_spinning_mabd_rolling_contact_candidate_config(
        self,
        tmpdir: str,
    ) -> Path:
        config = yaml.safe_load(ROLLING_SPINNING_CONFIG_PATH.read_text(encoding="utf-8"))
        config["mabd_rolling_contact_candidate"]["step_count"] = 4
        config["mabd_rolling_contact_candidate"]["sample_count"] = 3
        path = Path(tmpdir) / "single_body_rolling_spinning_short_mabd_rolling_contact.yaml"
        path.write_text(yaml.safe_dump(config), encoding="utf-8")
        return path
    ...
    def test_run_rolling_spinning_mabd_rolling_contact_candidate_writes_report(
        self,
    ) -> None:
        from mabd_reproduction.experiment_runner import (
        ...
        with TemporaryDirectory() as tmpdir:
            config_path = self._write_short_rolling_spinning_mabd_rolling_contact_candidate_config(
                tmpdir
            )
            output_path = Path(tmpdir) / "rolling_spinning_mabd_rolling_contact_candidate.json"
        ...
        self.assertEqual(
            loaded.observed["generated_plane_constraint_count_summary"]["max"],
            0,
        )
    ...
    def test_run_rolling_spinning_rbd_explicit_source_gate_writes_report(
        self,
    ) -> None:
        from mabd_reproduction.experiment_runner import (
        ...
        with TemporaryDirectory() as tmpdir:
            output_path = Path(tmpdir) / "rolling_spinning_rbd_explicit_source_gate.json"
            result = run_rolling_spinning_rbd_explicit_source_gate(
                config_path=ROLLING_SPINNING_CONFIG_PATH,
                matrix_path=MATRIX_PATH,
                output_path=output_path,
                source_commit="test-source",
        ...
        self.assertEqual(result.claim_id, "experiment.single_body.rolling_spinning")
        self.assertEqual(result.status, EvidenceStatus.INCOMPLETE)
        self.assertEqual(loaded.baseline_lane, "rbd_explicit_source_gate")
        self.assertEqual(
            loaded.solver_mode,
            "rolling_spinning_explicit_rbd_source_gate",
        )
        ...
        self.assertFalse(loaded.observed["paper_faithful_gate_passed"])
        self.assertFalse(loaded.observed["paper_comparable"])
        self.assertFalse(loaded.observed["full_experiment_claim_passed"])
        self.assertEqual(
        ...
        self.assertEqual(
            loaded.timing_distribution["status"],
            "not_measured",
        )
        self.assertEqual(
            loaded.timing_distribution["scope"],
    ...
    def test_run_rolling_spinning_rbd_implicit_source_gate_writes_report(
        self,
    ) -> None:
        from mabd_reproduction.experiment_runner import (
        ...
        with TemporaryDirectory() as tmpdir:
            output_path = Path(tmpdir) / "rolling_spinning_rbd_implicit_source_gate.json"
            result = run_rolling_spinning_rbd_implicit_source_gate(
                config_path=ROLLING_SPINNING_CONFIG_PATH,
                matrix_path=MATRIX_PATH,
                output_path=output_path,
                source_commit="test-source",
        ...
        self.assertEqual(result.claim_id, "experiment.single_body.rolling_spinning")
        self.assertEqual(result.status, EvidenceStatus.INCOMPLETE)
        self.assertEqual(loaded.baseline_lane, "rbd_implicit_source_gate")
        self.assertEqual(
            loaded.solver_mode,
    ...
    def test_run_rolling_spinning_mabd_source_gate_writes_report(
        self,
    ) -> None:
        from mabd_reproduction.experiment_runner import (
        ...
        with TemporaryDirectory() as tmpdir:
            output_path = Path(tmpdir) / "rolling_spinning_mabd_source_gate.json"
            result = run_rolling_spinning_mabd_source_gate(
                config_path=ROLLING_SPINNING_CONFIG_PATH,
                matrix_path=MATRIX_PATH,
                output_path=output_path,
                source_commit="test-source",
        ...
        self.assertEqual(result.claim_id, "experiment.single_body.rolling_spinning")
        self.assertEqual(result.status, EvidenceStatus.INCOMPLETE)
        self.assertEqual(loaded.baseline_lane, "mabd_source_gate")
        self.assertEqual(loaded.solver_mode, "rolling_spinning_mabd_source_gate")
        self.assertEqual(loaded.backend, "paper_source_audit")
        self.assertEqual(loaded.status, EvidenceStatus.INCOMPLETE)
        self.assertEqual(
            loaded.observed["source_audit_status"],
    ...
    def test_run_rolling_spinning_paper_timing_protocol_writes_report(
        ...
        with TemporaryDirectory() as tmpdir:
            output_path = Path(tmpdir) / "rolling_spinning_timing_protocol.json"
        ...
        self.assertEqual(loaded.backend, "report_protocol")
        self.assertEqual(loaded.expected["paper_comparable"], True)
        self.assertFalse(loaded.expected["full_experiment_claim_passed"])
        self.assertEqual(
            loaded.expected["paper_total_simulation_time_ms"]["explicit_rbd"],
            32.0,
        )
        self.assertEqual(
            loaded.observed["timing_protocol_status"],
            "paper_timing_protocol_incomplete",
        )
        self.assertTrue(loaded.observed["local_runtime_inputs_recorded"])
        self.assertFalse(loaded.observed["paper_comparable"])
        self.assertFalse(loaded.observed["full_experiment_claim_passed"])
        self.assertEqual(len(loaded.observed["input_reports"]), 5)
        for input_report in loaded.observed["input_reports"]:
            self.assertEqual(
                set(input_report),
                {
                    "path",
    ...
    def test_run_experiment_cli_runs_rolling_spinning_mabd_rolling_contact_candidate_lane(
        self,
    ) -> None:
        import json
        import subprocess
    ...
    def test_run_experiment_cli_runs_rolling_spinning_rbd_explicit_source_gate_lane(
        self,
    ) -> None:
        import json
        import subprocess
        ...
        self.assertEqual(payload["claim_id"], "experiment.single_body.rolling_spinning")
        self.assertEqual(payload["status"], "incomplete")
        self.assertEqual(payload["baseline_lane"], "rbd_explicit_source_gate")
        self.assertEqual(
            loaded.solver_mode,
            "rolling_spinning_explicit_rbd_source_gate",
        )
    ...
    def test_run_experiment_cli_runs_rolling_spinning_rbd_implicit_source_gate_lane(
        self,
    ) -> None:
        import json
        import subprocess
        ...
        self.assertEqual(payload["claim_id"], "experiment.single_body.rolling_spinning")
        self.assertEqual(payload["status"], "incomplete")
        self.assertEqual(payload["baseline_lane"], "rbd_implicit_source_gate")
        self.assertEqual(
            loaded.solver_mode,
            "rolling_spinning_implicit_rbd_source_gate",
        )
    ...
    def test_run_experiment_cli_runs_rolling_spinning_mabd_source_gate_lane(
        self,
    ) -> None:
        import json
        import subprocess
        ...
        self.assertEqual(payload["claim_id"], "experiment.single_body.rolling_spinning")
        self.assertEqual(payload["status"], "incomplete")
        self.assertEqual(payload["baseline_lane"], "mabd_source_gate")
        self.assertEqual(loaded.solver_mode, "rolling_spinning_mabd_source_gate")
        self.assertEqual(loaded.backend, "paper_source_audit")
        self.assertFalse(loaded.observed["paper_faithful_gate_passed"])
        self.assertFalse(loaded.observed["paper_comparable"])
    ...
    def test_run_spinning_box_contact_collision_gate_candidate_writes_report(
        self,
    ) -> None:
        from mabd_reproduction.experiment_runner import (
        ...
        with TemporaryDirectory() as tmpdir:
            config_path = self._write_short_spinning_box_contact_collision_gate_candidate_config(
                tmpdir
            )
            output_path = Path(tmpdir) / "spinning_box_contact_collision_gate_candidate.json"
        ...
        self.assertEqual(
            loaded.observed["unilateral_contact_policy"],
            "dense_cpu_active_set_drop_tensile_plane_rows",
        )
        self.assertFalse(loaded.observed["paper_faithful"])
        self.assertFalse(loaded.observed["paper_comparable"])
        self.assertFalse(loaded.observed["full_experiment_claim_passed"])
        self.assertFalse(loaded.observed["comparison_pass_gate_enabled"])
        self.assertEqual(loaded.observed["duration_s"], 0.02)
        self.assertEqual(loaded.observed["time_step_s"], 0.01)
        self.assertEqual(loaded.observed["step_count"], 2)
        self.assertEqual(loaded.observed["sample_count"], 3)
        self.assertEqual(loaded.observed["contact_constraint_mode"], "unilateral_plane")
        self.assertIn("phase88_rollout_candidate_report", loaded.observed)
        self.assertIn("phase88_rollout_candidate_sha256", loaded.observed)
        self.assertIn("max_unilateral_plane_requested_count", loaded.observed)
        self.assertIn("max_unilateral_plane_accepted_count", loaded.observed)
        self.assertIn("max_unilateral_plane_rejected_count", loaded.observed)
        self.assertIn("max_unilateral_plane_skipped_count", loaded.observed)
        self.assertEqual(len(loaded.observed["trajectory_samples"]), 3)
        self.assertIn(
        ...
        self.assertFalse(loaded.timing_distribution["paper_comparable"])
        self.assertEqual(loaded.plot_paths, {})
        self.assertIn("contact/collision gate candidate", loaded.failure_reason)
    ...
    def test_run_experiment_cli_writes_spinning_box_contact_collision_gate_candidate_report(
        self,
    ) -> None:
        import json
        import os
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            output_path = Path(tmpdir) / "contact_collision_gate_candidate_cli.json"
            result = subprocess.run(
                [
                    sys.executable,
                    "scripts/run_experiment.py",
