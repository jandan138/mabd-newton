...
class ExperimentRunConfigTests(unittest.TestCase):
    ...
    def test_spinning_box_affine_static_plane_contacts_rollout_candidate_config_is_development_only(
        self,
    ) -> None:
        config = load_spinning_box_config(ROOT / "configs/experiments/single_body_spinning_box.yaml")
        matrix = load_experiment_matrix(ROOT / "configs/experiments/paper_experiment_matrix.yaml")
        ...
        self.assertEqual(candidate.rollout_scope, "development_only")
        ...
        self.assertEqual(candidate.contact_constraint_mode, "plane")
        self.assertIn("max_constraint_residual_norm", candidate.thresholds)

        source = self._config_mapping()
        source["affine_static_plane_contacts_rollout_candidate"] = {
            "output_report": (
                "reports/experiment_matrix/"
                "single_body_spinning_box_affine_static_plane_contacts_rollout_candidate.json"
            ),
        ...
        invalid_paths = (
            ...
            "/tmp/single_body_spinning_box_affine_static_plane_contacts_rollout_candidate.json",
            (
                "reports/not_matrix/"
                "single_body_spinning_box_affine_static_plane_contacts_rollout_candidate.json"
            ),
            ...
            (
                "reports/experiment_matrix/"
                "not_the_spinning_box_affine_static_plane_contacts_rollout_candidate.json"
            ),
            (
                "reports/experiment_matrix/"
                "single_body_spinning_box_affine_static_plane_contacts_rollout_candidate.txt"
            ),
        )
        for invalid_path in invalid_paths:
            with self.subTest(invalid_path=invalid_path):
                invalid = deepcopy(source)
                invalid["affine_static_plane_contacts_rollout_candidate"][
                    "output_report"
                ] = invalid_path
                with TemporaryDirectory() as tmpdir:
                    path = self._write_config(tmpdir, invalid)
                    with self.assertRaisesRegex(
                        ExperimentRunConfigError,
                        "affine_static_plane_contacts_rollout_candidate.output_report",
                    ):
                        bad_config = load_spinning_box_config(path)
                        validate_spinning_box_config_against_matrix(bad_config, matrix)
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(
                ExperimentRunConfigError,
                "affine_static_plane_contacts_rollout_candidate.rollout_scope",
            ):
                load_spinning_box_config(path)

        invalid = deepcopy(source)
        invalid["affine_static_plane_contacts_rollout_candidate"]["paper_faithful"] = True
        with TemporaryDirectory() as tmpdir:
            path = self._write_config(tmpdir, invalid)
            with self.assertRaisesRegex(
                ExperimentRunConfigError,
                "affine_static_plane_contacts_rollout_candidate.paper_faithful",
            ):
                load_spinning_box_config(path)
    ...
    def test_rolling_spinning_config_is_machine_checkable(self) -> None:
        ...
        self.assertEqual(config.rbd_implicit_baseline.radius_m, 0.5)
        self.assertEqual(config.rbd_implicit_baseline.half_height_m, 0.5)
        self.assertEqual(config.rbd_implicit_baseline.density_kg_m3, 1000.0)
        self.assertEqual(config.rbd_implicit_baseline.time_step_s, 0.01)
        self.assertEqual(config.rbd_implicit_baseline.step_count, 10000)
        self.assertEqual(config.rbd_implicit_baseline.sample_count, 7)
        np.testing.assert_allclose(
        ...
        self.assertEqual(
            set(config.rbd_implicit_baseline.contact),
            {"ke", "kd", "kf", "mu", "gap"},
        )
        self.assertIn("max_no_slip_residual_m_s", config.rbd_implicit_baseline.thresholds)
    ...
    def test_rolling_spinning_rbd_no_slip_reference_is_machine_checkable(self) -> None:
        ...
        self.assertEqual(lane.radius_m, config.rbd_implicit_baseline.radius_m)
        self.assertEqual(lane.half_height_m, config.rbd_implicit_baseline.half_height_m)
        self.assertEqual(lane.density_kg_m3, config.rbd_implicit_baseline.density_kg_m3)
        ...
        np.testing.assert_allclose(lane.initial_linear_velocity_m_s[1:], [0.0, 0.0])
        np.testing.assert_allclose(lane.initial_angular_velocity_rad_s[:2], [0.0, 0.0])
        self.assertAlmostEqual(
        ...
        self.assertIn("max_no_slip_residual_m_s", lane.thresholds)
    ...
    def test_rolling_spinning_rbd_explicit_source_gate_is_fail_closed(
        self,
    ) -> None:
        config = load_rolling_spinning_config(ROLLING_SPINNING_CONFIG_PATH)
        matrix = load_experiment_matrix(ROOT / "configs/experiments/paper_experiment_matrix.yaml")
        ...
        self.assertEqual(
            lane.current_evidence_reports["rbd_explicit_baseline"],
            config.rbd_explicit_baseline.output_report,
        )
        self.assertEqual(
        ...
        invalid_paths = (
            ...
            config.rbd_explicit_baseline.output_report,
            config.rbd_explicit_no_slip_candidate.output_report,
            "/tmp/single_body_rolling_spinning_rbd_explicit_source_gate.json",
            "reports/not_matrix/single_body_rolling_spinning_rbd_explicit_source_gate.json",
            "../reports/experiment_matrix/single_body_rolling_spinning_rbd_explicit_source_gate.json",
            "reports/experiment_matrix/not_the_rolling_spinning_rbd_explicit_source_gate.json",
            "reports/experiment_matrix/single_body_rolling_spinning_rbd_explicit_source_gate.txt",
        )
        for invalid_path in invalid_paths:
            with self.subTest(invalid_path=invalid_path):
                invalid = replace(
                    config,
                    rbd_explicit_source_gate=replace(
                        config.rbd_explicit_source_gate,
                        output_report=invalid_path,
                    ),
                )
                with self.assertRaisesRegex(
                    ExperimentRunConfigError,
                    "rbd_explicit_source_gate.output_report",
                ):
        ...
        with self.assertRaisesRegex(
            ExperimentRunConfigError,
            "rbd_explicit_source_gate.required_source_parameters",
        ):
            validate_rolling_spinning_config_against_matrix(invalid, matrix)
        ...
        invalid = replace(
            ...
            rbd_explicit_source_gate=replace(
                ...
                current_evidence_reports={
                    "rbd_explicit_no_slip_candidate": config.rbd_explicit_no_slip_candidate.output_report,
                },
            ),
        )
        with self.assertRaisesRegex(
            ExperimentRunConfigError,
            "rbd_explicit_source_gate.current_evidence_reports",
        ):
            validate_rolling_spinning_config_against_matrix(invalid, matrix)

    def test_rolling_spinning_rbd_implicit_source_gate_is_fail_closed(
        self,
    ) -> None:
        config = load_rolling_spinning_config(ROLLING_SPINNING_CONFIG_PATH)
        matrix = load_experiment_matrix(ROOT / "configs/experiments/paper_experiment_matrix.yaml")
        ...
        self.assertEqual(
            lane.current_evidence_reports["rbd_implicit_baseline"],
            config.rbd_implicit_baseline.output_report,
        )
        self.assertEqual(
            config.required_missing_lanes,
        ...
        invalid_paths = (
            ...
            "/tmp/single_body_rolling_spinning_rbd_implicit_source_gate.json",
            "reports/not_matrix/single_body_rolling_spinning_rbd_implicit_source_gate.json",
            "../reports/experiment_matrix/single_body_rolling_spinning_rbd_implicit_source_gate.json",
            "reports/experiment_matrix/not_the_rolling_spinning_rbd_implicit_source_gate.json",
            "reports/experiment_matrix/single_body_rolling_spinning_rbd_implicit_source_gate.txt",
        )
        for invalid_path in invalid_paths:
            with self.subTest(invalid_path=invalid_path):
                invalid = replace(
                    config,
                    rbd_implicit_source_gate=replace(
                        config.rbd_implicit_source_gate,
                        output_report=invalid_path,
                    ),
                )
                with self.assertRaisesRegex(
                    ExperimentRunConfigError,
                    "rbd_implicit_source_gate.output_report",
                ):
                    validate_rolling_spinning_config_against_matrix(invalid, matrix)
        ...
        with self.assertRaisesRegex(
            ExperimentRunConfigError,
            "rbd_implicit_source_gate.required_source_parameters",
        ):
            validate_rolling_spinning_config_against_matrix(invalid, matrix)
        ...
        with self.assertRaisesRegex(
            ExperimentRunConfigError,
            "rbd_implicit_source_gate.current_evidence_reports",
        ):
            validate_rolling_spinning_config_against_matrix(invalid, matrix)
        ...
        invalid_lanes = (
            (
                ...
                replace(
                    ...
                    output_report="reports/experiment_matrix/not_the_no_slip_reference.json",
                ),
            ),
            ("time_step_s", replace(config.rbd_no_slip_reference, time_step_s=0.02)),
            ("step_count", replace(config.rbd_no_slip_reference, step_count=9999)),
            ("sample_count", replace(config.rbd_no_slip_reference, sample_count=2)),
            (
                "initial_position_m",
                replace(
                    config.rbd_no_slip_reference,
            ...
            (
                ...
                replace(
                    config.rbd_no_slip_reference,
                    initial_linear_velocity_m_s=np.array([1.0, 0.0, 0.1]),
                ),
            ),
            (
            ...
            (
                ...
                replace(
                    config.rbd_no_slip_reference,
                    initial_angular_velocity_rad_s=np.array([0.0, 0.1, -2.0]),
                ),
            ),
            (
                "no-slip",
    ...
    def test_rolling_spinning_rbd_implicit_baseline_report_path_must_be_lane_specific(
        self,
    ) -> None:
        config = load_rolling_spinning_config(ROLLING_SPINNING_CONFIG_PATH)
        matrix = load_experiment_matrix(ROOT / "configs/experiments/paper_experiment_matrix.yaml")
        ...
        invalid_paths = (
            config.output_report,
            "/tmp/single_body_rolling_spinning_rbd_implicit_baseline.json",
            "reports/not_matrix/single_body_rolling_spinning_rbd_implicit_baseline.json",
            "../reports/experiment_matrix/single_body_rolling_spinning_rbd_implicit_baseline.json",
            "reports/experiment_matrix/not_the_rolling_spinning_rbd_implicit_baseline.json",
            "reports/experiment_matrix/single_body_rolling_spinning_rbd_implicit_baseline.txt",
        )
        for invalid_path in invalid_paths:
            with self.subTest(invalid_path=invalid_path):
                invalid = replace(
                    config,
                    rbd_implicit_baseline=replace(
    ...
    def test_spinning_box_affine_static_plane_contacts_report_path_must_be_lane_specific(self) -> None:
        matrix = load_experiment_matrix(ROOT / "configs/experiments/paper_experiment_matrix.yaml")
        config = load_spinning_box_config(ROOT / "configs/experiments/single_body_spinning_box.yaml")
        ...
        invalid_paths = (
            ...
            config.paper_horizon.decoupled_twist_output_report,
            config.paper_horizon.figure_curve_output_report,
            "reports/experiment_matrix/not_the_spinning_box_affine_static_plane_contacts.json",
            "reports/experiment_matrix/single_body_spinning_box_affine_static_plane_contacts.txt",
        )
        for invalid_path in invalid_paths:
            with self.subTest(invalid_path=invalid_path):
                invalid = replace(
                    ...
                    paper_horizon=replace(
                        config.paper_horizon,
                        affine_static_plane_contacts_output_report=invalid_path,
                    ),
                )
                with self.assertRaisesRegex(
                    ExperimentRunConfigError,
